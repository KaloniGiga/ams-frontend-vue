...
const useArtistStore = defineStore('artist', () => {
    ...
    const postArtist = async (body) => {
        const { data } = await useFetch(`${API_BASE}/artists`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        if (data) {
            console.log(data);
    ...
    const putArtist = async (body) => {
        const { data } = await useFetch(`${API_BASE}/artists`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        if (data) {
    ...
    const deleteArtist = async () => {
        const { data } = await useFetch(`${API_BASE}/artists`, { method: "DELETE", headers: { "Content-Type": "application/json" } })
        if (data) {
            const targetUserIndex = artists.value.findIndex((item) => item.email !== data.value.email)
            if (targetUserIndex) {
